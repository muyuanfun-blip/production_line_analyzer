...
- [x] 修正首頁第二層深色工作區的淺色主題對比
  - [x] 為工作區分頁、深色內容卡、權限標籤、說明文字與次要按鈕建立專用語意類別
  - [x] 確保淺色模式的全域前景覆蓋不會降低深色工作區內文字與圖示對比
  - [x] 補上首頁工作區深色容器的淺色主題回歸測試
- [x] 診斷並修復 AI 分析功能失效
  - [x] 檢查前端提交狀態、有效功能權限、分析輸入資料與五角色結果處理
  - [x] 檢查雲端模型呼叫、逾時、回應格式與錯誤轉譯
  - [x] 修正可重現問題並提供可讀的失敗／待釐清狀態
  - [x] 補上 AI 分析服務可用性與權限路徑的回歸測試
...
## AI 分析服務韌性修復（完成）
- [x] 將五角色獨立審查改為併行執行，降低整體等待時間與前端連線中斷風險
- [x] 為五角色審查與互動分析加入 75 秒受控逾時與一次自動重試
- [x] 將模型逾時、驗證、服務忙碌與回傳格式錯誤轉為友善中文提示，並提供頁面內重試按鈕
- [x] 修復本地 Ollama 可用性測試未受控逾時問題，新增 AI 韌性單元測試
- [x] 驗證雲端 AI 服務端點可連線（HTTP 200），完整 Vitest 203 項通過

## 一般使用者 AI 分析權限（進行中）
- [x] 讓一般使用者的預設功能權限包含 AI 分析與分析報告匯出
- [x] 補上一般使用者可分析、不可治理或管理帳號的 RBAC 回歸測試
- [x] 驗證並保存權限調整版本

## AI 報告追溯資訊（進行中）
- [x] 在 AI 分析資料模型保留執行者與分析時間
- [x] 在頁面預覽、HTML 匯出與列印報告顯示分析執行者與時間戳記
- [x] 補上報告追溯資訊的單元測試並驗證保存版本

## IE 分析工具缺口盤點（進行中）
- [x] 盤點現有系統對 IE 核心工具與決策場景的涵蓋度
- [x] 對照標準 IE 能力建立功能缺口與導入優先順序
- [x] 提出可落地的分期導入建議

## 數位工時研究與標準工時（進行中）
- [x] 建立工時研究、觀測樣本與標準工時版本的資料模型及遷移
- [x] 實作正常工時、寬放與標準工時計算及工站套用規則
- [x] 建立工站工時研究與標準工時版本管理介面
- [x] 將已發布標準工時整合至平衡與產能分析的比較脈絡
- [x] 補齊單元測試、驗證並保存功能版本

## 手機行動版本（進行中）
- [x] 盤點現有手機響應式介面並定義現場作業優先流程
- [x] 建立手機優先的首頁、快速操作與底部導覽模式
- [x] 優化工時觀測、工站查閱與 AI 分析的觸控操作介面
- [x] 驗證手機尺寸下的可用性、可讀性與回歸測試

## 獨立手機入口與桌面分流（進行中）
- [x] 建立專屬手機入口路由與桌面管理版的分流規則
- [x] 建立不載入桌面側欄的手機現場作業殼層與底部導覽
- [x] 串接手機入口至工時觀測、工站查閱、平衡與 AI 分析流程
- [x] 補上入口與導覽規則測試並驗證保存版本

## 桌面與手機模式切換（進行中）
- [x] 在桌面版提供前往手機現場作業入口的明確連結
- [x] 在手機版提供返回桌面管理版的明確連結
- [x] 補上模式切換路徑測試並驗證保存版本

## 手機深層路由 404 修正（進行中）
- [x] 修正 /mobile/lines/:id 等深層手機網址的應用程式路由匹配
- [x] 補上手機深層網址路由回歸測試
- [x] 驗證直接開啟特定產線手機網址不再顯示 404 並保存版本

## 手機深層頁面過場與載入提示（進行中）
- [x] 在手機現場版深層路由切換加入短暫平滑的內容過場
- [x] 在工站、平衡、工時與 AI 資料載入期間提供清楚的行動提示
- [x] 尊重 prefers-reduced-motion 並補上過場狀態測試與驗證
