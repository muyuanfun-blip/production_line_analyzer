...
export function buildAIProfessionalReport(input: AIProfessionalReportInput) {
  ...
  return {
    ...
    analysisMetadata: input.analysisMetadata ?? { executorName: "系統未識別", executedAt: input.generatedAt },
...
export function buildAIProfessionalReportHtml(report: AIProfessionalReport): string {
  ...
  const analysisExecutedAt = report.analysisMetadata.executedAt.toLocaleString("zh-TW");
