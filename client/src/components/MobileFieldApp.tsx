...
function MobileFieldBottomNavigation({ location, navigate, userName }: { location: string; navigate: (path: string) => void; userName?: string | null }) {
  const items = [{ label: "首頁", path: "/mobile", icon: Activity }, { label: "產線", path: "/mobile/lines", icon: Factory }, { label: "任務", path: "/mobile/tasks", icon: ListChecks }, { label: "更多", path: "/mobile/more", icon: MoreHorizontal }];
  return <nav className="fixed inset-x-0 bottom-0 z-30 flex border-t border-border bg-background/95 px-2 pb-[max(0.5rem,env(safe-area-inset-bottom))] pt-2 shadow-[0_-8px_20px_rgba(0,0,0,.14)] backdrop-blur" aria-label="手機現場作業導覽">{items.map((item) => { const Icon = item.icon; const active = item.path === "/mobile" ? location === "/mobile" : location.startsWith(item.path); return <button key={item.path} onClick={() => navigate(item.path)} className={`flex min-h-14 flex-1 flex-col items-center justify-center gap-1 rounded-lg text-[11px] font-medium ${active ? "bg-primary/12 text-primary" : "text-muted-foreground"}`} aria-current={active ? "page" : undefined}><Icon className="h-5 w-5" /><span>{item.label}</span></button>; })}</nav>;
}
...
function MobileFieldHome() {
  ...
  const { data: latest = [] } = trpc.snapshot.getAllLinesLatestByDate.useQuery();
  const activeLines = lines.filter((line: any) => line.status === "active");
  const focus = (latest as any[]).filter((item) => item.snapshot).sort((a, b) => Number(a.snapshot.balanceRate) - Number(b.snapshot.balanceRate))[0];
  const focusLine = focus ? { id: focus.lineId, name: focus.lineName, balanceRate: Number(focus.snapshot.balanceRate) } : activeLines[0];
  const focusBalanceRate = (focusLine as any)?.balanceRate;
  return <div className="space-y-5"><section className="rounded-2xl border border-cyan-400/20 bg-[radial-gradient(circle_at_top_right,rgba(34,211,238,.16),transparent_42%),linear-gradient(135deg,rgba(15,23,42,.98),rgba(8,32,44,.96))] p-5"><p className="text-xs font-semibold uppercase tracking-[0.16em] text-cyan-300">現場作業入口</p><h1 className="mt-2 text-2xl font-bold text-white">{user?.name ? `${user.name}，開始作業` : "開始現場作業"}</h1><p className="mt-2 text-sm leading-6 text-slate-300">此入口僅提供行動現場流程；完整管理、配置與治理功能請使用桌面版。</p>{focusLine ? <button onClick={() => setLocation(`/mobile/lines/${focusLine.id}`)} className="mt-4 flex w-full items-center justify-between rounded-xl border border-white/10 bg-black/15 p-4 text-left"><span><small className="text-slate-300">優先產線</small><b className="mt-1 block text-white">{focusLine.name}</b>{focusBalanceRate !== undefined && <small className="mt-1 block text-cyan-200">平衡率 {Number(focusBalanceRate).toFixed(1)}%</small>}</span><ChevronRight className="h-5 w-5 text-cyan-300" /></button> : <Button className="mt-4 w-full" onClick={() => setLocation("/mobile/lines")}>選擇產線</Button>}</section><div className="grid grid-cols-2 gap-3"><Metric label="運行中產線" value={`${activeLines.length} 條`} color="text-cyan-400" /><Metric label="可用現場流程" value="4 項" color="text-emerald-400" /></div><section><h2 className="font-semibold">快速開始</h2><div className="mt-3 grid grid-cols-2 gap-3">{[{ label: "工時觀測", icon: Activity, path: focusLine ? `/mobile/lines/${focusLine.id}/time-study` : "/mobile/lines", tone: "text-cyan-400" }, { label: "工站查閱", icon: Factory, path: focusLine ? `/mobile/lines/${focusLine.id}` : "/mobile/lines", tone: "text-emerald-400" }, { label: "平衡摘要", icon: BarChart3, path: focusLine ? `/mobile/lines/${focusLine.id}/balance` : "/mobile/lines", tone: "text-violet-400" }, { label: "AI 分析", icon: Brain, path: focusLine ? `/mobile/lines/${focusLine.id}/ai` : "/mobile/lines", tone: "text-amber-400" }].map((item) => <button key={item.label} onClick={() => setLocation(item.path)} className="min-h-28 rounded-xl border border-border bg-card p-4 text-left active:scale-[0.98]"><item.icon className={`h-5 w-5 ${item.tone}`} /><b className="mt-3 block text-sm">{item.label}</b></button>)}</div></section></div>;
}
...
function EmptyMobile({ title, detail }: { title: string; detail: string }) { return <div className="rounded-xl border border-dashed border-border p-8 text-center"><CheckCircle2 className="mx-auto h-8 w-8 text-muted-foreground/40" /><p className="mt-3 font-medium">{title}</p><p className="mt-1 text-sm text-muted-foreground">{detail}</p></div>; }
