...
export default function Home() {
  ...
  return <div className="decision-home min-h-full space-y-5 p-4 sm:p-6">
    <section className="home-reveal relative overflow-hidden rounded-2xl border border-cyan-400/20 bg-[radial-gradient(circle_at_top_right,rgba(34,211,238,.12),transparent_34%),linear-gradient(135deg,rgba(15,23,42,.95),rgba(10,33,46,.92))] p-5 sm:p-7"><div className="relative z-10 flex flex-col gap-6 xl:flex-row xl:items-end xl:justify-between"><div className="max-w-2xl"><div className="flex items-center gap-2 text-xs font-semibold uppercase tracking-[0.18em] text-cyan-300"><Activity className="h-4 w-4" />決策中樞</div><h1 className="mt-3 text-2xl font-bold tracking-tight sm:text-3xl">產線改善決策儀表板</h1><p className="mt-2 text-sm leading-6 text-muted-foreground">先掌握跨產線營運訊號，再進入產能、品質與資料、治理、改善四個工作區處理下一步。</p></div><div className="flex flex-wrap gap-2"><Button onClick={() => setLocation("/lines")}><Factory className="mr-2 h-4 w-4" />檢視產線</Button>{isAdmin && <Button variant="outline" onClick={() => setLocation("/admin/ai-consensus-governance")}><ShieldCheck className="mr-2 h-4 w-4" />處理治理事項</Button>}</div></div></section>
