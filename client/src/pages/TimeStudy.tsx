    ...
    <div className="space-y-5 p-4 sm:p-6">
      ...
      <div className="grid gap-6 xl:grid-cols-[360px,minmax(0,1fr)]">
        ...
        {!selectedStudy ? <Card className="border-dashed border-border"><CardContent className="p-12 text-center"><Calculator className="mx-auto mb-3 h-12 w-12 text-muted-foreground/30" /><h2 className="text-lg font-semibold">選擇或建立一份工時研究</h2><p className="mt-2 text-sm text-muted-foreground">完成觀測、評比與寬放設定後，即可發布為該工站目前有效的標準工時。</p></CardContent></Card> : <div className="space-y-6">
          <Card className="border-border bg-card">
            <CardHeader className="flex flex-col items-stretch justify-between gap-4 space-y-0 sm:flex-row sm:items-start"><div><div className="flex flex-wrap items-center gap-2"><CardTitle>{selectedStudy.name}</CardTitle><Badge className={`${studyStatusMeta(selectedStudy.status).className} border`}>{studyStatusMeta(selectedStudy.status).label}</Badge></div><CardDescription className="mt-2">工站：{selectedStudy.workstationName} · 版本 V{selectedStudy.versionNumber}{selectedStudy.productVariant ? ` · 型號／條件：${selectedStudy.productVariant}` : ""}</CardDescription></div><div className="grid grid-cols-2 gap-2 sm:flex">{isEditable && <Button variant="outline" size="sm" onClick={() => recalculate.mutate({ timeStudyId: selectedStudy.id })} disabled={recalculate.isPending}><RefreshCw className="mr-1.5 h-3.5 w-3.5" />重算</Button>}{isEditable && <Button size="sm" onClick={() => publish.mutate({ timeStudyId: selectedStudy.id })} disabled={publish.isPending || selectedStudy.sampleCount < 3}><Upload className="mr-1.5 h-3.5 w-3.5" />發布</Button>}</div></CardHeader>
            <CardContent>
          ...
          <Card className="border-border bg-card">
            ...
            <CardContent className="space-y-4">
              {isEditable && <div className="grid gap-3 rounded-lg border border-border bg-background/30 p-4 md:grid-cols-[minmax(0,1fr)_minmax(0,1fr)_minmax(0,2fr)_auto]"><div><Label className="text-xs">觀測時間（秒）</Label><Input className="mt-1 min-h-11" inputMode="decimal" type="number" min="0.01" step="0.01" value={observationForm.observedCycleTime} onChange={(event) => setObservationForm((form) => ({ ...form, observedCycleTime: event.target.value }))} placeholder="例：18.50" /></div><div><Label className="text-xs">個別評比（選填）</Label><Input className="mt-1 min-h-11" inputMode="decimal" type="number" min="0.5" max="1.5" step="0.01" value={observationForm.performanceRating} onChange={(event) => setObservationForm((form) => ({ ...form, performanceRating: event.target.value }))} placeholder={`預設 ${asNumber(selectedStudy.defaultPerformanceRating).toFixed(2)}`} /></div><div><Label className="text-xs">備註（選填）</Label><Input className="mt-1 min-h-11" value={observationForm.notes} onChange={(event) => setObservationForm((form) => ({ ...form, notes: event.target.value }))} placeholder="例：正常節拍、熟練作業員" /></div><div className="flex items-end"><Button className="min-h-11 w-full md:w-auto" onClick={handleAddObservation} disabled={addObservation.isPending}><Plus className="mr-1.5 h-4 w-4" />加入</Button></div></div>}
