...
import { DESKTOP_ENTRY_PATH, getEntrySwitchPath, getMobileLineFlowPath, isMobileEntryPath, MOBILE_ENTRY_PATH } from "../shared/mobileEntry";
...
describe("獨立手機入口路徑", () => {
  ...
  it("提供桌面與手機入口的雙向切換路徑", () => {
    expect(getEntrySwitchPath("/lines/12/balance")).toBe(MOBILE_ENTRY_PATH);
    expect(getEntrySwitchPath("/mobile/lines/12/time-study")).toBe(DESKTOP_ENTRY_PATH);
  });
});
